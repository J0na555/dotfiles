...
hl.bind(
	...
	hl.dsp.exec_cmd(
		"uwsm app -- " .. terminal,
		{ float = true, size = { 800, 500 }, move = { "(monitor_w*0.2)", "(monitor_h*0.1)" } }
	)
)
...
hl.bind(
	mainMod .. "+ CONTROL + B",
	hl.dsp.exec_cmd(
		"killall waybar || uwsm app -- waybar -c ~/.config/waybar/config.jsonc -s ~/.config/waybar/styles.css"
	)
)
...
hl.bind(
	mainMod .. " + CONTROL + R",
	hl.dsp.exec_cmd([[sh -c 'wf-recorder -g "$(slurp)" -f ~/Videos/recording_$(date +%Y-%m-%d_%H-%M-%S).mp4']])
)
-- Stop recording
...
hl.bind(
	"Print",
	hl.dsp.exec_cmd("hyprshot -m output -t 1500 -o ~/Pictures/Screenshots -f $(date +%F_%H-%M-%S)_hyprshot.png")
)

-- region selection
hl.bind(
	"SHIFT + Print",
	hl.dsp.exec_cmd("hyprshot -m region -t 1500 -o ~/Pictures/Screenshots -f $(date +%F_%H-%M-%S)_hyprshot.png")
)
...
hl.bind(
	mainMod .. " + Z",
	hl.dsp.exec_cmd(
		"hyprctl keyword cursor:zoom_factor $(awk \"BEGIN {print $(hyprctl getoption cursor:zoom_factor | grep 'float:' | awk '{print $2}') + 0.3}\")"
	),
	{ repeating = true }
)
hl.bind(
...
hl.bind(
	...
	hl.dsp.exec_cmd("~/.config/bin/volume-control.sh d"),
	{ repeating = true, locked = true }
)
hl.bind(
	"XF86AudioRaiseVolume",
	hl.dsp.exec_cmd("~/.config/bin/volume-control.sh i"),
	{ repeating = true, locked = true }
)
...
hl.bind(
	...
	hl.dsp.exec_cmd("~/.config/bin/brightness-control.sh -o d"),
	{ repeating = true, locked = true }
)
hl.bind(
	"XF86MonBrightnessUp",
	hl.dsp.exec_cmd("~/.config/bin/brightness-control.sh -o i"),
	{ repeating = true, locked = true }
)
